...
def a2j_init(keypointsNumber=15, model_dir='/home/yxs/Yang/pose_estimation/depth/common/A2J/model/ITOP_side_ver0.pth'):
    ...
    try:
        while True:
            if online:
                ...
                depth_image = np.load('data/depth_seq{:02d}/{:05d}.npy'.format(1, idx+1))
                color_image = cv2.imread('data/color_seq{:02d}/{:05d}.jpg'.format(1, idx+1))
            
            depth_image = depth_image.astype(float) / 1000.0

            # Person detection
            bbox_list = person_detection(yolov3_model, color_image, imgsz, device, half)  # shape = [num_people, 4]
            ...
            # Record data
            # if detected:
            #     cv2.imwrite(os.path.join(save_path, 'color_seq{:02d}/{:05d}.jpg'.format(seq_num, idx)), color_image)
